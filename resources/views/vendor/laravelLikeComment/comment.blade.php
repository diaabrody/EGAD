    ...
    <div class="ui threaded comments" id="{{ $comment_item_id }}-comment-0">
        ...
        <form class="ui laravelComment-form form" id="{{ $comment_item_id }}-comment-form" data-parent="0" data-item="{{ $comment_item_id }}" style="display: none;">
            <div class="field">
                ...
                @if(!Auth::check())
                    <small>من فضلك سجل الدخول لتتمكن من وضع تعليق علي البلاغ</small>
                @endif
            </div>
            <input type="submit" class="ui basic small submit button" value="تعليق" {{ $GLOBALS['commentDisabled'] }}>
        </form>
<?php
...
$comments = \App\Http\Controllers\Frontend\Comment\CommentsController::getComments($comment_item_id);
